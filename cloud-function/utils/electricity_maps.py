...
class Client:
    ...
    def get_carbon_intensity(self, zone: str) -> dict:
        return self._make_request(f"{ELECTRICITYMAPS_BASE_URL}/carbon-intensity/latest?zone={zone}")

    def get_power_breakdown(self, zone: str) -> dict:
        return self._make_request(f"{ELECTRICITYMAPS_BASE_URL}/power-breakdown/latest?zone={zone}")
...
def test_get_carbon_intensity():
    ...
    assert isinstance(response["carbonIntensity"], int)
    assert isinstance(response["datetime"], str)
